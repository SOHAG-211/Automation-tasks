...
export default class echoPageMob {
      [x: string]: any;
      ...
      async ClickBusM() {
            ...
            const ele = this.page.locator("(//a[@title='Bus'])[2]")
            expect(ele).toBeEnabled()
            await ele.click()
      }
      async inputFrom() {
  ...
  async clickDOJDate() {
   ...
   // changes here for mobile automation using same method
  async clickDOR(event:string) {
        const locator = this.page.locator(event)
        expect(locator).toBeVisible()
        await locator.click()
  }
  async clickDORDate() {
  ...
  async ClickLaunch() {
    ...
    const ele = this.page.locator("(//a[@title='Launch'])[2]")
    expect(ele).toBeEnabled()
    await ele.click()
}
async inputLTo() {
